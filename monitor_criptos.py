...
def generar_señal_rsi(rsi: float, precio_actual: float, historico) -> dict:
    """Señal combinada RSI + MACD(σ) + tendencia + persistencia + métricas DIP (más agresiva)."""
    try:
...
def enviar_telegram(mensaje: str):
    ...
    try:
        ...
        if "parse" in body.lower() or "entity" in body.lower():
            ...
            r2.raise_for_status()
            return
        if "chat not found" in body.lower(): logging.error("Verifica TELEGRAM_CHAT_ID.")
        if "bot was blocked" in body.lower(): logging.error("El usuario bloqueó al bot.")
        if "message is too long" in body.lower(): logging.error("Mensaje supera 4096 caracteres; recórtalo.")
        r.raise_for_status()
    except requests.exceptions.RequestException:
    ...
    try:
        ...
        for moneda in MONEDAS:
            try:
                ...
                macd_raw     = ind.get("macd_raw", macd_val)
                ...
                if macd_val is not None and macd_sig is not None:
                    ...
                    if delta is not None and vol is not None:
                        eps = 0.2 * MACD_SIGMA_K * max(1e-12, vol)  # eps coherente con recomendación agresiva
